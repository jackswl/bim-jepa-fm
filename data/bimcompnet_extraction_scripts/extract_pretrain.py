        ...
        try:
            ...
            with requests.get(url, headers=headers, stream=True, timeout=30) as r:
                if r.status_code != 206:
                    continue  # Server didn't give partial content, retry
                data = r.content
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Stream-download .obj files from a remote BIMCompNet pretrain zip "
                    "(named as Instance.obj)."
    )
    parser.add_argument("url", help="Remote ZIP URL (e.g. https://host/Museum.zip).")
