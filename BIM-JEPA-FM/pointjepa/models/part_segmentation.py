...
class PointJepaPartSegmentation(pl.LightningModule):
    def __init__(
        ...
        # Metrics are built in setup(), where num_classes is available from the datamodule
        self.train_acc = None
        self.val_acc = None
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.train_acc = Accuracy(task="multiclass", num_classes=self.num_seg_classes)
        self.val_acc = Accuracy(task="multiclass", num_classes=self.num_seg_classes)
        self.val_macc = Accuracy(task="multiclass", num_classes=self.num_seg_classes, average="macro")
    ...
    def configure_optimizers(self):
        ...
        # Treat "encoder" (Transformer) AND "tokenizer" (PointNet) as the Backbone
        ...
        enc_lr = self.hparams.encoder_learning_rate if self.hparams.encoder_learning_rate is not None else self.hparams.learning_rate
        ...
        opt = torch.optim.AdamW(
            ...
            lr=self.hparams.learning_rate,  # global fallback/scheduler base
        ...
        warmup_scheduler = LinearLR(
            opt,
            start_factor=self.hparams.lr_scheduler_linear_warmup_start_lr / self.hparams.learning_rate,  # type: ignore
            end_factor=1.0,
    ...
    def on_train_epoch_start(self) -> None:
        if self.hparams.freeze_encoder:
            # explicit freeze requested, do nothing
            return 
