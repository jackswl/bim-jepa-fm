...
class PointcloudJitter(nn.Module):
    def __init__(self, sigma: float = 0.01, clip: float = 0.05):
        ...
        - sigma: std-dev of the noise in normalized coordinate units
        - clip:  clamp noise to [-clip, clip]
        """
        super().__init__()
        self.sigma = sigma
        self.clip = clip
    ...
    def forward(self, points: torch.Tensor):
        # points: (B, N, C), we only jitter XYZ (first 3 dims)
        B, N, C = points.shape
        noise = torch.randn(B, N, 3, device=points.device) * self.sigma
