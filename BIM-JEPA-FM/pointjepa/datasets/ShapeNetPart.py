...
class ShapeNetPart(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            print(f"Error loading {self.datapath[index][1]}: {e}. Skipping...")
            try:
                parent_dir = os.path.dirname(self.datapath[index][1])
...
class ShapeNetPartDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if self.hparams.use_validation_split:
            # Mode: Train on 'train', Val on 'val'
            self.train_dataset = ShapeNetPart(self.hparams.data_dir, split="train")
            self.val_dataset = ShapeNetPart(self.hparams.data_dir, split="val")
            self.test_dataset = ShapeNetPart(self.hparams.data_dir, split="test")
        else:
            # Mode: Train on 'train' + 'val', Val on 'test'
            self.train_dataset = ShapeNetPart(self.hparams.data_dir, split="trainval")
            # In this mode, we use the test set for validation during training
            self.val_dataset = ShapeNetPart(self.hparams.data_dir, split="test")
            self.test_dataset = ShapeNetPart(self.hparams.data_dir, split="test")
